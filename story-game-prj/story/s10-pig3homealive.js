// 클래스명 바꿔주기
export default class Pig3HomeAlive{
  ...
  constructor() {
    ...
    this.img = new Image();
    this.img.src = "img/S10.jpg";
    
    // 오디오 파일 불러오기 (src 임시) , .loop 자동반복 false로 막음
    this.audio = new Audio();
    this.audio.src = "audio/sound/scene10.mp3";
    ...
    this.subtitleX = 500;
    this.subtitleY = 250; /** 자막 위치 */
    this.lineInterval = 40; /** 자막 간격 */
    this.subtitleTxt = [' "후우 훅!" 그런데 이게 웬일이지?',
                        '벽돌로 튼튼하게 만든 집은 꿈쩍도 하지 않았어요.']; /*여기다가 멘트들 한줄씩 넣으면 됨*/
